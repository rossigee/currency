...
**CRITICAL**: All new wizard dialogs MUST follow this pattern:
- **NO header buttons** - Remove `<header>` section entirely from wizard views
- **Save button handles final action** - Override `create()` method to trigger action when Save is clicked
- **Validation controls Save availability** - Use computed fields to disable Save when invalid
- **Consistent UX** - Users expect Save button to perform the wizard's primary function

Example implementation:
```python
def create(self, vals):
    """Override create to execute action when Save is clicked"""
    record = super().create(vals)
    return record.action_do_primary_task()
```

This ensures:
- ✅ Clean interface without confusing header buttons
- ✅ Standard Odoo Save/Cancel workflow
- ✅ Validation prevents invalid submissions
- ✅ Consistent user experience across all wizards

### Recently Implemented (Phase 7: PSBT Support)
**Problem**: Need PSBT (Partially Signed Bitcoin Transaction) workflow for multisig coordination
**Solution**: Complete BIP-174 compliant PSBT implementation:
- Transaction creation wizard with output specification
- BIP-174 compliant PSBT generation using `embit` library
- PSBT export/import with proper binary file handling
- Transaction status tracking (draft → unsigned → partial → signed → confirmed)
- Standalone Import PSBT menu item for external PSBT files
- Proper PSBT validation (magic bytes + structure validation)
